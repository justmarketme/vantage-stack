...
export async function ensureCrmSchema(db: Sql) {
  ...
  await Promise.all([
    ...
    db.unsafe(`
      ...
      alter table public.clients add column if not exists serve_area text;
      alter table public.clients add column if not exists team_size text;
      ...
      alter table public.clients add column if not exists website_enrichment jsonb;
